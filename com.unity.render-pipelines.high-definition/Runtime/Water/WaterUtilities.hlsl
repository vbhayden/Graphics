...
{
#if defined(WATER_QUAD_GEOMETRY)
    ...
    float3 normalWS, float3 positionAWS, out FoamData foamData)
{
    ...
    foamTex = pow(foamTex, lerp(2.0, 0.75, saturate(lowFrequencyHeight)));
    ...
    float3 topFoam = saturate((simulationFoam.y + customFoam) * foamMask.y * _SurfaceFoamIntensity * _WindFoamAttenuation * foamTex);
...
float VoronoiNoise(float2 coordinate, float2 period)
{
...
float EvaluateCaustics(float3 bedPositionAWS)
{
    ...
    return  VoronoiNoise(causticsUV, 20.0) * normalizedDepth * _CausticsIntensity;
}
